...
from PyQt5.QtMultimedia import QMediaPlayer,QMediaPlaylist,QMediaContent,QSoundEffect#添加音频控制
...
class Timer(QMainWindow, Ui_MainWindow1):
    ...
    def initUI(self):
        ...
        self.label_show.setStyleSheet("QLabel{background:rgb(0, 0, 0,50);}"
    ...
    def showTime(self):#
        ...
        if int(run_time/60) < C:#5:#此处10代表计划时长的实例#此处没问题
            ...
            self.label.setStyleSheet("QLabel{background:rgb(0, 0, 0,100);}"
                                          "QLabel{color:rgb(92,255,6, 250); font-size:80px; font-weight:bold}")
            ...
            self.label.setStyleSheet("QLabel{background:rgb(0, 0, 0,200);}"
                                     "QLabel{color:rgb(92,255,6, 250); font-size:80px; font-weight:bold}")
            # 红色超时示警
            # song = QMediaContent(QUrl.fromLocalFile("1.mp3"))
            # self.playlist.addMedia(song)
            # self.player.play()

            url = self.QUrl.fromLocalFile("1.mp3")
